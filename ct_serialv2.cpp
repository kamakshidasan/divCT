...
#define divy 2
#define divz 2
#define sub_sizex 128
#define sub_sizey 128
#define sub_sizez 128
...
long long int* vis_asc;
long long int* vis_desc;
...
int XYZ = 8;
int XYZ2 = 16;
int neighbors[] = { 2, 3, 4, 5, 2, 3, 4, 5, 0, 1, 4, 5, 0, 1, 4, 5, 0, 1, 2, 3,
		0, 1, 2, 3 };
int bodyAdj[] = { 1, 2, 4, 0, 3, 5, 0, 3, 6, 1, 2, 7, 0, 5, 6, 1, 4, 7, 2, 4, 7,
		3, 5, 6 };
...
void classifyCriticalPoints() {
	...
	for (int v = 0; v < num_vert; v++) {
		...
		int * bnd = bndc;
		int b[36] = { 1, 2, 3, 4, 5, 0, 0, 2, 3, 4, 5, 1, 3, 0, 1, 4, 5, 2, 2,
				0, 1, 4, 5, 3, 5, 0, 1, 2, 3, 4, 4, 0, 1, 2, 3, 5 };
...
int find_min_root(int i, int source, int *steep_desc) {
	int k = i;
	while(vis_desc[k] == -1) {
			if (k == steep_desc[k])
				{
				vis_desc[k] = source;
				return extrema_map[k];
...
int find_max_root(int i, int source, int *steep_asc) {
	int k = i;
		while(vis_asc[k] == -1) {
			if (k == steep_asc[k])
				{
...
void find_roots(){
	...
	int s = vertex_index[i];
		int c = vertex_index[i]*MAX_ADJ;
			int c1 = c;
			for (int j =0; j<lower_root_count[s]; j++) {
				//lower_roots[i*MAX_ADJ+j] = extrema_map[find_min_root(lower_roots[i*MAX_ADJ+j], steep_desc)];
				lower_roots[c] = find_min_root(lower_roots[c],s, steep_desc);
				//lower_roots[c] = extrema_map[lower_roots[c]];
				c ++;
			}
			}
			delete[] vis_desc;
	for(int i =num_critical-1;i>=0;i--){
			
			int s = vertex_index[i];
			int c = vertex_index[i]*MAX_ADJ;
			for (int j =0; j<upper_root_count[s]; j++) {
				// upper_roots[i*MAX_ADJ+j] = extrema_map[find_max_root(upper_roots[i*MAX_ADJ+j], steep_asc)];
				upper_roots[c] = find_max_root(upper_roots[c],s, steep_asc);
...
//	float * data = new float[num_vert];
//	FILE* fp = fopen(file, "r+b");
//	fread(data, sizeof(float), num_vert, fp);
//	fclose(fp);

vertices = new float[num_vert * 2];
vis_asc = new long long int[num_vert];
vis_desc = new long long int[num_vert];
...
if (argc != 7) {
	cerr << "Usage: ./ct input_file dimx dimy dimz output_file_id subcube_no." << endl;
	exit(1);
}
...
ext.z[1] =(((scz + 1) * sub_sizez) < global_dimz) ?
