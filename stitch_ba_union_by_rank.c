...
int final;
int critical;
int Union(int child, int parent, int * Uk) {
	...
	else if (rank[parent]<rank[child])
	{
		Uk[parent] = child;
		Uk[child] = -1;
		return child;
...
void cleanup_trees() {
	...
	for (i = 0; i < num_critical; i++) {
		...
		long long int dimxy = dim * dim;
				long long int posz = vpos / dimxy;
				long long int xy = vpos % dimxy;
				long long int posy = xy / dim;
				long long int posx = xy % dim;
...
strcat(file, ".dat");
	...
	read(fp1, (void*) (&dim), isz);
	read(fp2, (void*) (&dim), isz);
	read(fp1, (void*) (ext1), 6 * isz);
	read(fp2, (void*) (ext2), 6 * isz);

	//extents
	ext[0] = min(ext1[0], ext2[0]);
...
//exit(0);
	...
	Rep = (int*) malloc((n1 + n2) * sizeof(int));
	rankj = (int*) malloc((n1 + n2) * sizeof(int));
	ranks = rankj;
